...
class SlipReportpdf extends Component
{
    public function generatePDF($id, $date)
    {
       ...
       $allownce  = Allownce::where('user_id','=',$id)->where("date", 'LIKE', $date . '-%')->get();
        ...
        // $mpdf->WriteHTML(
            view('livewire.salaries.SlipReport',["salary"=>$salary,"allownce"=>$allownce, "deduction"=>$deduction,'checks' => $checks,'employee' => $employee,'employee_id' => $employee_id,'company' => $company,'image' => $image,'department' => $department,'position' => $position,'date'=>$date])
        // )
        ;
